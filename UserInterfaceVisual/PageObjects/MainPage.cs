...
namespace UserInterfaceVisual.pageObjects
{
    public class MainPage : Form
    {
        ...
        private ITextBox EmailTextBox => ElementFactory.GetTextBox(By.XPath("//a[@class='start__link']"), "First Card Link");
        private IButton CookiePanel => ElementFactory.GetButton(By.XPath("//div[@class='cookies']"), "Cookies Panel");
        private IButton AcceptCookiesButton => ElementFactory.GetButton(By.XPath("(//button[normalize-space()='Not really, no'])[1]"), "Accept Cookies button");
        private ITextBox TimerValue => ElementFactory.GetTextBox(By.XPath("//div[@class='timer timer--white timer--center']"), "Timer");
        private IButton HideHelpFormButton => ElementFactory.GetButton(By.XPath("//button[contains(@class, 'send-to-bottom')]"), "Hide Form Button");

        public void getFirstCard() 
        {
            EmailTextBox.State.WaitForClickable();
            if (EmailTextBox.State.IsDisplayed)
            {
                EmailTextBox.Click();
            }
